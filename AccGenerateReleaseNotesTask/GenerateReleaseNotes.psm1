...
function Expand-WorkItemData {
    ...
    try {
        if ($showParents -eq $false) {
            ...
            # Filter the wi's based on Empty RN
            $keys = @($wiArray.Keys)
            Write-Verbose "        Filtering WI on empty rn "
            foreach ($key in $keys) {
                $wi = $wiArray.$key
                $wiRn = $wi.fields."DeloitteFinTech.SolvasScrum.ReleaseNotes"
                if ([string]::IsNullOrEmpty($wiRn) -eq $false ) { 
                    $wiRn = $wiRn.Trim()
                }

                if (([string]::IsNullOrEmpty($wiRn) -eq $true) -or $wiRn -eq "NA" -or $wiRn -eq "N/A"  ) {
                    Write-Verbose "        Removed WI $($key) as does not have release notes"
                    $wiArray.Remove($key)
...
function Invoke-GetWithHeaderCommand {
    [CmdletBinding()]
    param
    (
        $uri,
    ...
    #write-verbose "REST Call [$uri]"
    $cont = $webclient.DownloadString($uri)
    $result = @{ header = $webclient.ResponseHeaders ;
        content         = $cont
    }
    $result
}
...
function Get-BuildDataSet {
    ...
    if (($build.repository.type -eq "TfsGit") -and ![string]::IsNullOrEmpty($activateFix) -and ($activateFix.ToLower() -eq $true)) {
        ...
        $workItemData = Get-WorkItemDataFromChangeSets -tfsUri $tfsUri -teamproject $teamproject -changesets $changesets -usedefaultcreds $usedefaultcreds -maxItems $maxWi
        $buildWorkItems = Get-WorkItemDataFromBuild -tfsUri $tfsUri -teamproject $teamproject -buildid $buildid -usedefaultcreds $usedefaultcreds -maxItems $maxWi
        foreach ($wi in  $buildWorkItems.value ) {
            # Add-Member -InputObject $wi -MemberType NoteProperty -Name url -Value "$($tfsUri)_apis/wit/workItems/$($wi.id)";
            $workItemData.value += $wi
        }
        $workItemData.count = $workItemData.value.length
    }
