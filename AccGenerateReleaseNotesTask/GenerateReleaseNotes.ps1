...
param (
    ...
    $unifiedList,
    [parameter(Mandatory = $false, HelpMessage = "Publish location")]
    $publishLoc,
    [parameter(Mandatory = $false, HelpMessage = "Release Notes location")]
    $notesRootPath
)
...
Import-Module -Name "$PSScriptRoot\Transformers.psm1" -Force 
...
Write-Verbose "publishLoc = [$publishLoc]"
Write-Verbose "notesRootPath = [$notesRootPath]"
...
if ($unifiedWorkItems.count -ne 0 -or $builds.workitems.count -ne 0 -or $releases.workitems.count -ne 0 ) {
    
    Write-host " Writing Notes"
    $writeScriptPath = $env:ABS_RN_WriteScript
    if ($null -eq $writeScriptPath) {
        $writeScriptPath = ".Write.ps1" 
    }
    .(Join-Path (pwd) Write.ps1)
}
else { 
